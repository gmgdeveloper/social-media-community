...
if (!function_exists('get_user_image')) {
    function get_user_image($file_name_or_user_id = "", $optimized = "") {
        ...
        if ($user_id > 0) {
            ...
            // Check if the file name exists and construct the URL accordingly
            if (!empty($file_name)) {
                if (File::exists(public_path('/storage/userimage/' . $optimized . $file_name))) {
                    return url('/storage/userimage/' . $optimized . $file_name);
                }
            }
        } else {
            // Check if the file name exists and construct the URL accordingly
        ...
        // Return the default image URL if the image is not found
        return url('/storage/userimage/default.png');
    }
}
...
if (!function_exists('get_guest_image')) {
    function get_guest_image() {
        if (auth()->user()->user_role === 'Guest') {
            return url('/storage/userimage/guest_user.png');
        } else {
            return url('/storage/userimage/default.png');
        }
    }
...
if (! function_exists('get_cover_photo')) {
    function get_cover_photo($file_name_or_user_id = '', $optimized = "") {
        ...
        if ($user_id > 0) {
            ...
            // Check if the file name exists and construct the URL accordingly
            if (!empty($file_name)) {
                if (File::exists(public_path('/storage/cover_photo/' . $optimized . $file_name))) {
                    return url('/storage/cover_photo/' . $optimized . $file_name);
                }
            }
        } else {
            // Check if the file name exists and construct the URL accordingly
        ...
        // Return the default image URL if the image is not found
        return url('storage/cover_photo/default.jpg');
    }
}
...
if (! function_exists('get_album_thumbnail')) {
    function get_album_thumbnail($id = '', $optimized = "") {
        ...
        if(!empty($file_name) && File::exists('public/storage/thumbnails/album/'.$optimized.$file_name)){
            return url('storage/thumbnails/album/'.$optimized.$file_name);
        }
        ...
        if(!empty($file_name) && File::exists('public/storage/post/images/'.$optimized.$file_name)){
            return url('storage/post/images/'.$optimized.$file_name);
        }else{
            return url('storage/thumbnails/album/default.jpg');
        }
    }
}
...
//             return url('storage/post/images/'.$optimized.$file_name);
//         }else{
//             return url('storage/post/images/default.jpg');
//         }
//     }
// }
if (!function_exists('get_post_image')) {
    function get_post_image($file_name = '', $optimized = "")
    {
        // If the file name starts with 'https://', it's an external link, so return it directly
        if (strpos($file_name, 'https://') !== false) {
            return $file_name;
        }

        // Set the optimized directory
        $optimized = ($optimized !== "") ? $optimized . '/' : '';

        // Construct the full path to the image
        $image_path = public_path('storage/post/images/' . $optimized . $file_name);
        ...
        if (file_exists($image_path)) {
            ...
            // Return the URL to the default image if the specified image doesn't exist
            return url('storage/post/images/default.jpg');
        }
    }
}
...
if (! function_exists('get_post_video')) {
    function get_post_video($file_name = '', $optimized = "") {
        ...
        if(File::exists('public/storage/post/videos/'.$optimized.$file_name)){
            return url('storage/post/videos/'.$optimized.$file_name);
        }else{
            return url('storage/post/videos/default.jpg');
        }
    }
}
...
//         $video_path = public_path('/storage/post/videos/' . $optimized . $file_name);
...
//         } else {
//             // Return the URL to the default video if the specified video doesn't exist
//             return url('/storage/post/videos/default.jpg');
//         }
//     }
// }
...
if (! function_exists('viewImage')) {
    function viewImage($foldername = "",$imagename = "",$imagetype = "") {
        if (!empty($foldername)&&!empty($imagename)&&!empty($imagetype)){
            return url('storage/'.$foldername.'/'.$imagetype.'/'.$imagename);
        }else{
           return url('storage/'.$foldername.'/'.$imagetype.'/default/default.jpg');
        }
    }
}
    ...
    function get_product_image($file_name = "", $foldername = "") {
        ...
        if(!empty($file_name)){
            if(File::exists('public/storage/marketplace/'.$foldername.$file_name)){
                return url('storage/marketplace/'.$foldername.$file_name);
            }else{
                return url('storage/marketplace/'.$foldername.'default/default.jpg');
            }
        }else{
            return url('storage/marketplace/'.$foldername.'default/default.jpg');
        }
    ...
    function get_sponsor_image($file_name = "", $foldername = "") {
        ...
        if(!empty($file_name)){
            if(File::exists('public/storage/sponsor/'.$foldername.$file_name)){
                return url('storage/sponsor/'.$foldername.$file_name);
            }else{
                return url('storage/sponsor/'.$foldername.'default/default.jpg');
            }
        }else{
            return url('storage/sponsor/'.$foldername.'default/default.jpg');
        }
    ...
    function get_blog_image($file_name = "", $foldername = "") {
        ...
        if(!empty($file_name)){
            if(File::exists('public/storage/blog/'.$foldername.$file_name)){
                return url('storage/blog/'.$foldername.$file_name);
            }else{
                return url('storage/blog/'.$foldername.'default/default.jpg');
            }
        }else{
            return url('storage/blog/'.$foldername.'default/default.jpg');
        }
    ...
    function get_page_logo($file_name = "", $foldername = "") {
        ...
        if(!empty($file_name)){
            if(File::exists('public/storage/pages/'.$foldername.$file_name)){
                return url('storage/pages/'.$foldername.$file_name);
            }else{
                return url('storage/pages/'.$foldername.'default/default.jpg');
            }
        }else{
            return url('storage/pages/'.$foldername.'default/default.jpg');
        }
    ...
    function get_group_logo($file_name = "", $foldername = "") {
        ...
        if(!empty($file_name)){
            if(File::exists('public/storage/groups/'.$foldername.$file_name)){
                return url('storage/groups/'.$foldername.$file_name);
            }else{
                return url('storage/groups/'.$foldername.'default/default.jpg');
            }
        }else{
            return url('storage/groups/'.$foldername.'default/default.jpg');
        }
...
if (! function_exists('get_group_cover_photo')) {
    function get_group_cover_photo($file_name = "", $foldername = "") {
        // Check if the file name is an external URL
        if(strpos($file_name, 'https://') !== false){
            return $file_name;
        }

        $foldername = $foldername.'/'; // Append a slash to the folder name

        // Construct the full path to the image
        $image_path = public_path('/storage/groups/' . $foldername . $file_name);

        // Check if the image file exists
        if(!empty($file_name) && File::exists($image_path)){
            // Return the URL to the image
            return url('/storage/groups/' . $foldername . $file_name);
        } else {
            // Return the URL to the default image if the image file doesn't exist
            return url('/storage/groups/' . $foldername . 'default/default.jpg');
        }
    }
}
    ...
    function get_system_logo_favicon($file_name = "", $foldername = "") {
        ...
        if(!empty($file_name)){
            if(File::exists('public/storage/logo/'.$foldername.$file_name)){
                return url('storage/logo/'.$foldername.$file_name);
            }else{
                return url('storage/logo/'.$foldername.'default/default.jpg');
            }
        }else{
            return url('storage/logo/'.$foldername.'default/default.jpg');
        }
