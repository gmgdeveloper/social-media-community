    ...
    function create_post(Request $request)
    {
        ...
        if (is_array($request->multiple_files) && $request->multiple_files[0] != null) {
            ...
            foreach ($request->multiple_files as $key => $media_file) {
                ...
                $file_name = uniqid() . '.' . $media_file->getClientOriginalExtension();

                // Determine file type based on extension
                $file_extension = strtolower($media_file->getClientOriginalExtension());
                $file_type = in_array($file_extension, ['jpeg', 'png', 'jpg', 'gif', 'svg']) ? 'image' : 'Video';

                // Determine the upload path based on file type
                $upload_path = ($file_type === 'image') ? 'storage/post/images/' : 'storage/post/videos/';

                // Move the uploaded file to the upload path
                $media_file->move(public_path($upload_path), $file_name);
    ...
    function edit_post($id, Request $request)
    {
        ...
        if (is_array($request->multiple_files) && $request->multiple_files[0] != null) {
            foreach ($request->multiple_files as $key => $media_file) {
                ...
                // Move the uploaded file to the upload path
                $media_file->move(public_path($upload_path), $file_name . '.' . $file_extension);
