    ...
    function create_story(Request $request)
{
    ...
    if ($request->content_type != 'text') {
        ...
        foreach ($request->story_files as $key => $media_file) {
            if (!empty($media_file)) {
                $file_extension = $media_file->getClientOriginalExtension();
                ...
                $upload_path = ($file_type === 'image') ? 'story/images' : 'story/videos';
    
                // Move the uploaded file to the upload path
                $media_file->move(public_path('storage/' . $upload_path), $file_name);
    
                // Debugging: Check the full path of the uploaded file
                $full_path = public_path('storage/' . $upload_path . '/' . $file_name);
