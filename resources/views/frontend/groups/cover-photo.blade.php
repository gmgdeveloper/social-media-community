    ...
    <div class="profile-header" style="background-image: url('{{get_group_cover_photo($group->banner,"coverphoto")}}')">
        ...
        <div class="profile-avatar d-flex align-items-center ps-4">
            ...
            <div class="avatar-details">
                ...
                @if ($group->user_id==auth()->user()->id)
                    <a href="javascript:void(0)" onclick="showCustomModal('{{route('load_modal_content', ['view_path' => 'frontend.groups.edit-modal', 'group_id' => $group->id])}}', '{{get_phrase('Edit Community')}}');" class="btn btn-primary mt-3" data-bs-toggle="modal"
                        data-bs-target="#edit-profile"><i class="fa fa-pen"></i>{{get_phrase('Edit Community')}}</a>
                @endif
            </div>
        </div>
    </div>
